...
namespace WpfTools
{
    ...
    public class ImageFromFont : MarkupExtension
    {
        ...
        {
            if (this.FontFamily != null && !String.IsNullOrEmpty(this.Text))
            {
                //premier essai, on charge la police directement
                Typeface typeface = new Typeface(this.FontFamily, this.Style, this.Weight, this.Stretch);

                GlyphTypeface glyphTypeface;
                if (!typeface.TryGetGlyphTypeface(out glyphTypeface))
                {
                    //si ça ne fonctionne pas (et pour le mode design dans certains cas) on ajoute l'uri pack://application
                    typeface = new Typeface(new FontFamily(new Uri("pack://application:,,,"), this.FontFamily.Source), this.Style, this.Weight, this.Stretch);
                    if (!typeface.TryGetGlyphTypeface(out glyphTypeface))
                        throw new InvalidOperationException("No glyphtypeface found");
                }

                //détermination des indices/tailles des caractères dans la police
                ushort[] glyphIndexes = new ushort[this.Text.Length];
                double[] advanceWidths = new double[this.Text.Length];
                ...
                for (int n = 0; n < this.Text.Length; n++)
                {
                    ...
                    try
                    {
                        glyphIndex = glyphTypeface.CharacterToGlyphMap[this.Text[n]];
                ...
                try
                {
                    ...
                    GlyphRunDrawing glyphRunDrawing = new GlyphRunDrawing(this.Brush, gr);
                    return new DrawingImage(glyphRunDrawing);
                }
                catch (Exception ex)
                {
